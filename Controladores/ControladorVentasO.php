...
class ControladorVentas{
    ...
    public static function ctrLlenarCarrito($item, $valor, $datosCarrito){
        ...
        $producto = ModeloProductos::mdlMostrarProductos($tabla, $item, $valor);
			...
			if($datosCarrito['tipo_desc'] == 'S/' && $descuentoGlobal > 0){
		...
		if($datosCarrito['tipo_desc'] == '%' && $descuentoGlobalP > 0){
		...
		$total = $op_gravadas + $op_exoneradas + $op_inafectas + $igv;
            ...
            echo "<script>
                    ...
                    $('.tabla-totales .totales .op-descuento-item').children().next().html('".$simboloMoneda.round($descuento_item_total,2)."');
	...
	//extract($_REQUEST);
		...
		if(empty($codigosSunat)){
			...
			$actualizarStock = ControladorProductos::ctrActualizarStock($detalle);
